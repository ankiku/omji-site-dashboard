...
// Public routes for shareable expense/payment/contact views
app.get('/api/public/projects/slug/:slug/expenses', async (req, res) => {
  const { slug } = req.params;
  try {
    const projects = await getProjects();
    const project = projects.find(p => p.slug === slug);
    if (!project) return res.status(404).json({ message: 'Project not found' });
    const data = await getModuleData('expenses', project.id);
    res.json(data);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
app.get('/api/public/projects/slug/:slug/payments', async (req, res) => {
  const { slug } = req.params;
  try {
    const projects = await getProjects();
    const project = projects.find(p => p.slug === slug);
    if (!project) return res.status(404).json({ message: 'Project not found' });
    const data = await getModuleData('payments', project.id);
    res.json(data);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
app.get('/api/public/projects/slug/:slug/contacts', async (req, res) => {
  const { slug } = req.params;
  try {
    const projects = await getProjects();
    const project = projects.find(p => p.slug === slug);
    if (!project) return res.status(404).json({ message: 'Project not found' });
    const data = await getModuleData('contacts', project.id);
    res.json(data);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
app.get('/api/public/projects/slug/:slug/materials', async (req, res) => {
  const { slug } = req.params;
  try {
    const projects = await getProjects();
    const project = projects.find(p => p.slug === slug);
    if (!project) return res.status(404).json({ message: 'Project not found' });
    const data = await getModuleData('materials', project.id);
    res.json(data);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
app.post('/api/storage/upload', authenticateToken, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const isBillUpload = ['expense-bill', 'material-bill', 'payment-bill'].includes(taskId);
    let outputFilename;
    let fileUrl;

    if (isBillUpload) {
      // Bill uploads: save original file as-is (no compression/conversion)
      const ext = path.extname(req.file.originalname) || (mimeType.startsWith('image/') ? '.jpg' : '');
      outputFilename = `${baseName}${ext}`;
      fs.writeFileSync(path.join(UPLOADS_DIR, outputFilename), req.file.buffer);
    } else if (mimeType.startsWith('image/')) {
      // Photo uploads: resize + convert to WebP for optimization
