...
import { addPhotoRecord, uploadPhoto, deletePhotoRecord } from '../services/localStorageService';
...
export default function PhotoLogTab({ photos, tasks, projectId, canEdit, onPhotoClick }) {
  ...
  const handleDeletePhoto = async (e, photoId) => {
    ...
    if (ok) {
      try {
        await deletePhotoRecord(projectId, photoId);
                ...
                <div className="client-photo-grid photo-grid" style={{ gap: '16px', display: 'grid' }}>
                  {groupedPhotos[date].map((photo) => {
                    ...
                    return (
                      ...
                      >
                        <div className="photo-thumb" style={{ height: '130px', overflow: 'hidden', position: 'relative' }}>
                          {canEdit && (
                            <button
                              onClick={(e) => handleDeletePhoto(e, photo.id)}
                              style={{ position: 'absolute', top: 6, right: 6, background: 'rgba(220, 38, 38, 0.85)', color: 'white', border: 'none', borderRadius: '50%', width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 10, fontSize: '14px' }}
                              title="Delete photo"
                            >✕</button>
                          )}
                          <LazyPhoto
